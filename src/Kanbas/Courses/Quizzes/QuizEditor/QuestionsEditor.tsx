    ...
    return (
        <div>
            ...
            <li className="wd-quiz-list-item list-group-item p-3 pb-5 ps-3 pe-4 mb-3 border rounded-2 w-75" key={question._id}>
                ...
                {!currentQuestion.type.includes("FILL_IN") && (<div>
                    {currentQuestion.choices.map((c: any) => (
                        <div key={c._id}>
                            {currentQuestion.type.includes("MULTIPLE CHOICE") && (
                                <div>
                                    <div className="d-flex w-100 mt-2">
                                        <input className="form-check-input ms-1" checked={c.isCorrect} value={c.choiceText} onChange={(e) => handleCorrectAnswerChange(c._id)} type="radio" name={currentQuestion.id} id={c._id}></input>
                            ...
                            {question.type.includes("TRUE/FALSE") && (
                                <div>
                                    <div className="d-flex w-100 mt-2">
                                        <input className="form-check-input ms-1" checked={c.isCorrect} value={c.choiceText} type="radio" name={currentQuestion._id} onChange={(e) => handleCorrectAnswerChange(c._id)} id={c._id}></input>
                                        <label className="form-check-label ps-2 d-flex w-100" htmlFor="flexRadioDefault1">
                                            {c.choiceText}
                                        </label>
                                    </div>
                ...
                <div className="d-flex float-start mt-2 mb-2">
                    ...
                    {(question.type.includes("MULTIPLE CHOICE")) && (
                        <button className="btn btn-primary" onClick={(e) => { handleAddAnswer(false) }}>Add Choice</button>
                    )}
                    {(question.type.includes("FILL IN")) && (
                        <button className="btn btn-primary" onClick={(e) => { handleAddAnswer(true) }}>Add Choice</button>
                    )}
                </div>
            </li> */
